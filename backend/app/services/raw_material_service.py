...
_RM_FIELDS = {"material_group_id", "setor_id", "unidade_conversao_id", "peso_liquido"}
...
def _to_response(item: Item) -> RawMaterialResponse:
    ...
    return RawMaterialResponse(
        ...
        supplier_ids=[s.id for s in rm.suppliers],
...
class RawMaterialService:
    ...
    def create(self, payload: RawMaterialCreate) -> RawMaterialResponse:
        ...
        supplier_objects = self._validate_suppliers(payload.supplier_ids)
        self._validate(payload.unit_of_measure_id, payload.material_group_id,
                       payload.setor_id, payload.unidade_conversao_id)
        data = payload.model_dump(exclude={"supplier_ids"})
        item_data = {k: v for k, v in data.items() if k in _ITEM_FIELDS}
        rm_data = {k: v for k, v in data.items() if k in _RM_FIELDS}
        item = self.repository.create(item_data, rm_data, supplier_objects)
    ...
    def update(self, item_id: UUID, payload: RawMaterialUpdate) -> RawMaterialResponse:
        ...
        self._validate(item.unit_of_measure_id, payload.material_group_id,
                       payload.setor_id, payload.unidade_conversao_id)
        data = payload.model_dump(exclude={"supplier_ids"}, exclude_none=True)
        item_data = {k: v for k, v in data.items() if k in _ITEM_FIELDS}
        rm_data = {k: v for k, v in data.items() if k in _RM_FIELDS}
        ...
        updated = self.repository.update(item_id, item_data, rm_data, supplier_objects)
    ...
    def _validate_suppliers(self, supplier_ids: list[UUID]) -> list[Supplier]:
        result = []
        for sid in supplier_ids:
            s = self.supplier_repo.get_by_id(sid)
            if s is None:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"Fornecedor não encontrado: {sid}",
                )
            result.append(s)
        return result
